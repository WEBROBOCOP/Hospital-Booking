...
import DoctorPatientAssignmentModal from '../components/DoctorPatientAssignmentModal';
import { 
  FaUsers, 
  FaUserMd, 
  FaUserInjured, 
...
const AdminDashboard = () => {
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      // If the created user is a doctor, automatically open facility assignment modal
      if (newUser.role === 'doctor') {
        setSelectedDoctor(createdUser);
  ...
  const handleDoctorAssignmentClick = (user) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/7">Name</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/7">Email</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/7">Role</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/7">Created</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/7">Facility</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/7">Assigned Doctor</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/7">Actions</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={user._id} className="hover:bg-gray-50">
                          ...
                          {user.role === "doctor" ? (
                            user.facility?.name ? (
                              <div className="truncate max-w-xs">
                                <div className="font-medium text-xs text-green-700">{user.facility.name}</div>
                                <div className="text-xs text-gray-400">{user.facility.type}</div>
                              </div>
                            ) : (
                              <div className="flex items-center space-x-2">
                                <span className="text-red-500 text-xs font-medium">No facility assigned</span>
                                <button
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500 w-1/6">
                          {user.role === "patient" ? (
                            user.assignedDoctor ? (
                              <div className="truncate max-w-xs">
                                <div className="font-medium text-xs text-green-700">Dr. {user.assignedDoctor.firstName} {user.assignedDoctor.lastName}</div>
                                <div className="text-xs text-gray-400">{user.assignedDoctor.specialty || 'General Practice'}</div>
                              </div>
                            ) : (
                              <div className="flex items-center space-x-2">
                                <span className="text-red-500 text-xs font-medium">No doctor assigned</span>
                                <button
                                  onClick={() => handleDoctorAssignmentClick(user)}
                                  className="text-blue-600 hover:text-blue-800 text-xs underline"
                                >
                                  Assign now
                                </button>
        ...
        {/* Doctor Patient Assignment Modal */}
        {showDoctorAssignmentModal && selectedPatient && (
          <DoctorPatientAssignmentModal
            patient={selectedPatient}
            onClose={() => {
              setShowDoctorAssignmentModal(false);
              setSelectedPatient(null);
            }}
            onSave={handleAssignDoctor}
          />
        )}
        {showUserModal && (
